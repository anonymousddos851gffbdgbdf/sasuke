...
TELEGRAM_TOKEN = '8219744399:AAED0KnuAyhxTjGHTjBEJ7uD_CKguHtSHsU'
...
async def server(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not github_tokens:
        await update.message.reply_text("Admin must set GitHub token(s) first with /token. Power By @Shadow_lScript")
        return
    if not approved_ids:
        await update.message.reply_text("No approved IDs to run attack on. Use /approve first. Power By @Shadow_lScript")
        return
    if len(context.args) != 3:
        await update.message.reply_text("Usage: /server <ip> <port> <time> Power By @Shadow_lScript")
        return
